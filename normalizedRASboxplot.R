...
normRASS <- read.table('all_ClustersNormalized.allFs.RASS.txt', header = T)
library(ggplot2)
library(reshape2)
...
p <- ggplot(subset(normRASS.m, cluster%in% c("US_34")),aes(x=variable, y=value, color=variable, fill=variable))+ geom_boxplot(alpha=0.4)+geom_jitter(position = position_jitter(0.05), size=rel(4), alpha=0.6)+facet_wrap(~cluster, ncol=2)+ scale_color_manual(values =cols) +scale_fill_manual(values =cols)#+ stat_summary(fun.data = "mean_sdl", fun.args=list(mult=1), geom="pointrange", color="black", alpha=0.2)
